/**
 * Project copy, verbatim from `content/project-blurbs.md`.
 *
 * Shared so the homepage's featured section and the `/projects` index render
 * the same text from one source rather than drifting apart.
 */
export type Project = {
  slug: string;
  title: string;
  /** One-line summary, used on its own in compact listings. */
  tagline: string;
  stack: string[];
  body: string[];
